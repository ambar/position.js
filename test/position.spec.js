...
describe('position', () => {
  ...
  it('positions `top-left`', () => {
    expect(position(big, little, 'top-left')).toMatchObject({left: 100, top: -100})
  })

  it('positions `top-right`', () => {
    expect(position(big, little, 'top-right')).toMatchObject({left: 0, top: -100})
  })

  it('positions `right-top`', () => {
    expect(position(big, little, 'right-top')).toMatchObject({left: 200, top: 100})
  })

  it('positions `right-bottom`', () => {
    expect(position(big, little, 'right-bottom')).toMatchObject({left: 200, top: 0})
  })

  it('positions `bottom-left`', () => {
    expect(position(big, little, 'bottom-left')).toMatchObject({left: 100, top: 200})
  })

  it('positions `bottom-right`', () => {
    expect(position(big, little, 'bottom-right')).toMatchObject({left: 0, top: 200})
  })

  it('positions `left-top`', () => {
    expect(position(big, little, 'left-top')).toMatchObject({left: -100, top: 100})
  })

  it('positions `left-bottom`', () => {
    expect(position(big, little, 'left-bottom')).toMatchObject({left: -100, top: 0})
  })
  ...
  describe('adjustXY option', () => {
    ...
    describe('horizontally OR vertically', () => {
      ...
      it('adjusts correctly', () => {
        expect(position(popup, anchor, 'bottom', options))
          .toMatchObject({left: 50, top: 200, placement: 'bottom'})

        expect(position(popup, anchor, 'top', options))
          .toMatchObject({left: 50, top: 200, placement: 'bottom'})

        expect(position(popup, anchor, 'left', options))
          .toMatchObject({left: 200, top: 50, placement: 'right'})
      })

      it('only transposes main direction', () => {
        expect(position(popup, anchor, 'top-left', options))
          .toMatchObject({left: 100, top: 200, placement: 'bottom-left'})

        expect(position(popup, anchor, 'top-right', options))
          .toMatchObject({left: 0, top: 200, placement: 'bottom-right'})
      })

      it('handles combos', () => {
        expect(
          position(popup, anchor, 'top-left', options).offset
        )
        .toEqual(
          position(popup, anchor, {popup: 'bottom-left', anchor: 'top-left'}, options).offset
        )
      })
    })
    ...
    describe('horizontally AND vertically', () => {
      ...
      it('adjusts `top` to `left`', () => {
        ...
        expect(position(popup, anchor, 'top', options))
          .toMatchObject({left: 700, top: 450, placement: 'left'})
      })

      it('adjusts `top-left` to `bottom-right`', () => {
        Object.assign(anchor, {left: bounds.width - anchor.width, top: 500})
        expect(position(popup, anchor, 'top-left', options))
          .toMatchObject({left: 800, top: 600, placement: 'bottom-right'})
      })

      it('adjusts `top-right` to `left-top`', () => {
        Object.assign(anchor, {left: bounds.width - anchor.width / 2, top: 500})
        expect(position(popup, anchor, 'top-right', options))
          .toMatchObject({left: 750, top: 500, placement: 'left-top'})
      })
      ...
      it('handles combos', () => {
        ...
        .toEqual(
          position(popup, anchor, {popup: 'top-right', anchor: 'bottom-right'}, options).offset
        )
      })
    })

    it('accepts array of placements', () => {
      expect(position(popup, anchor, ['top-left', 'top']))
        .toMatchObject({left: 100, top: -100, placement: 'top-left'})

      expect(position(popup, anchor, [null, 'top']))
        .toMatchObject({left: 50, top: -100, placement: 'top'})
